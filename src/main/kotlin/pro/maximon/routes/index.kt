...
fun Routing.index() {
    get("/{schedule}/{week}") {
        ...
        val schedule = MaiApi.getSchedule(group = Group(scheduleName))
            ?: run { call.response.status(HttpStatusCode.NotFound); return@get }
        val scheduleNameEncoded = java.net.URLEncoder.encode(scheduleName, "utf-8")

        val week = call.parameters["week"]?.toIntOrNull() ?: run {
            call.response.status(
                HttpStatusCode(
                    400,
                    "Week is not a number"
                )
            ); return@get
        }
        if (week !in schedule.getWeeks().map { it.number }) {
            call.response.status(HttpStatusCode.NotFound); return@get
        }
    ...
    get("/{schedule}") {
        ...
        val currentWeekId = allWeeks.find { it.range == currentWeek }
            ?: run { call.response.status(HttpStatusCode.NotFound); return@get }
